...
/**
 * Cavalry unit - fast moving melee unit with high damage
 */
class CavalryUnit(
    x: Float,
    y: Float,
...
) : UnitEntity(x, y, team, UnitEntity.UnitType.CAVALRY, race) {
    ...
    companion object {
        ...
        fun loadBitmap(context: Context) {
            ...
            if (!elfBitmapLoaded) {
                try {
                    val inputStream = context.assets.open("cavarlyelifbirimi.png")
            ...
            if (!mechanicalBitmapLoaded) {
                try {
                    val inputStream = context.assets.open("mekaniklejyonatlıbirimi.png")
                    val originalBitmap = android.graphics.BitmapFactory.decodeStream(inputStream)
                    inputStream.close()
