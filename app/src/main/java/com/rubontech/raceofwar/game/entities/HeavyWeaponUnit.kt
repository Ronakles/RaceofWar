...
class HeavyWeaponUnit(
    ...
    companion object {
        ...
        fun loadBitmap(context: Context) {
            ...
            if (!elfBitmapLoaded) {
                try {
                    val inputStream = context.assets.open("heavyweapnelfbirimi.png")
            ...
            if (!mechanicalBitmapLoaded) {
                try {
                    val inputStream = context.assets.open("mekaniklejyonheavyweaponbirimi.png")
                    val originalBitmap = android.graphics.BitmapFactory.decodeStream(inputStream)
                    inputStream.close()
    ...
    override fun performAttack(target: Entity) {
        // Heavy Weapon performs melee attack
