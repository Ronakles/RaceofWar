...
/**
 * Archer unit - long range, high damage, low HP
 */
class ArcherUnit(
    x: Float,
    y: Float,
...
) : UnitEntity(x, y, team, UnitEntity.UnitType.ARCHER, race) {
    ...
    companion object {
        ...
        fun loadBitmap(context: Context) {
            ...
            if (!elfBitmapLoaded) {
                try {
                    val inputStream = context.assets.open("archerbirimielf.png")
            ...
            if (!mechanicalBitmapLoaded) {
                try {
                    val inputStream = context.assets.open("mekaniklejyonuzakçıbirimi.png")
                    val originalBitmap = android.graphics.BitmapFactory.decodeStream(inputStream)
                    inputStream.close()
