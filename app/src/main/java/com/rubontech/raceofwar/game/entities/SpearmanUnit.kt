...
/**
 * Spearman unit - high HP unit with long range attacks
 */
class SpearmanUnit(
    x: Float,
    y: Float,
...
) : UnitEntity(x, y, team, UnitEntity.UnitType.SPEARMAN, race) {
    ...
    companion object {
        ...
        fun loadBitmap(context: Context) {
            ...
            if (!elfBitmapLoaded) {
                try {
                    val inputStream = context.assets.open("elfsperamanbirimi.png")
            ...
            if (!mechanicalBitmapLoaded) {
                try {
                    val inputStream = context.assets.open("mekaniklejyonmızrakçıbirimi.png")
                    val originalBitmap = android.graphics.BitmapFactory.decodeStream(inputStream)
                    inputStream.close()
